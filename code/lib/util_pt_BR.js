...
// Uma das possiveis funções que encontra ou constrou os quizzes na view por meio o arquivo js  com formato  Json
function buildQuizzes(searchTerm) {
  var quizzes = require("../data/quizzes_pt-BR");
  if (searchTerm) {
  ...
  for (var i = 0; i < quizzes.length; i++) {
    ...
    if (!questions || !questions.length) {
      console.log("O quiz escolhido não tem perguntas!");
    } else {
...
function arrayToListForSpeech(input) {
  ...
  output = output.substring(0, output.lastIndexOf("...")) + "... ou " + output.substring(output.lastIndexOf("...") + 3, output.length) + "..."
